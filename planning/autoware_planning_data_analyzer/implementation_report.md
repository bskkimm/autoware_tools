...
| **Collision candidate** | Valid candidate requires polygon overlap, no red-light pseudo-object token, and not already in proposal-local collided-track memory: $P_{i,t}\cap O_{t,o}\ne\varnothing$, `red_light_token not in o`, $o\notin C_{i,t}$. | Valid candidate is polygon overlap only: $P_t^{aw}\cap O_{t,o}^{aw}\ne\varnothing$. There is no NAVSIM red-light pseudo-object topic in `PredictedObjects`, and no collided-object memory. | **Partially unmatched** | Red-light exclusion is not needed for Autoware NC because those pseudo objects are absent. The missing collided-track memory is a real semantic gap. |
...
| **Front-bumper intersection** | $F_{i,t}=\mathrm{LineString}(P_{i,t}^{ext}[0],P_{i,t}^{ext}[3])$, then $F_{i,t}\cap O_{t,o}\ne\varnothing$. | $F_t^{aw}=\mathrm{LineString}(T_t(x_f,y_{min}),T_t(x_f,y_{max}))$ from `VehicleInfo` front and lateral offsets, then $F_t^{aw}\cap O_{t,o}^{aw}\ne\varnothing$. | **Mostly matched** | Same concept: line segment across the ego front bumper intersected with object polygon. Autoware constructs it explicitly from vehicle offsets. |
...
| **Full NAVSIM equation** | $\mathrm{NC}_{nav,i}=\min(\{1.0\}\cup\{\phi_{i,t,o}\mid(t,o)\in K_i\})$, with $K_i=\{(t,o)\mid\mathrm{ValidCollision}_{i,t,o}\}$. | Conceptual Autoware analogue is the same minimum form, but current C++ returns immediately on the first at-fault collision. | **Partially unmatched** | To match the minimum-over-events behavior, Autoware should keep scanning after a $0.5$ event and only safely early-return on $0.0$. |
...
=
\left[P_{i,t}\cap O_{t,o}\ne\varnothing\right]
\land
\left[\mathrm{red\_light\_token}\notin o\right]
\land
\left[o\notin C_{i,t}\right],
$$
...
$$
\mathcal{R}_t =
\{\ell\;|\;\ell\in\mathrm{RoadLaneletsAtPose}(\mathrm{pose}(A_t))
\land \mathrm{isRouteLanelet}(\ell)\}.
$$
...
=
\left[P_t^{aw}\cap \mathrm{polygon}(\ell)\ne\varnothing\right].
$$
...
- **Migrated local helpers:** `create_vehicle_footprints()`, `collect_candidate_lanelets()`, `fuse_lanelet_polygons()`
...
| :--- | :--- | :--- | :--- | :--- |
| **Drivable Area Map** | Definition of all legally/geometrically drivable space. | `lanelet::ConstLanelets drivable_lanelets` | The specific lanelets belonging to the active route. | **Moderate.** AW is stricter; it considers anywhere outside the *route* non-drivable. |
...
| **Ego-Area Classification** | Mask determining "ROAD" vs "NON-ROAD" status. | `fuse_lanelet_polygons()` | Union of route lanelets into one admissible polygon. | **Close.** Semantically similar, but restricted to the route set. |

- **Semantic replacement meaning:** the migration treats the route lanelet union as the drivable area proxy.
...
- The migrated code unions only the route-relevant lanelets reachable from the trajectory samples.
- This can be stricter or simply different near intersections, wide drivable zones, parking areas, or off-center maneuvers that are still within NAVSIM drivable polygons.
- **Impact:** **Moderate.** Same high-level intent, but not the same admissible region.
...
**Migrated Autoware inputs.** Let $A_t$ be the selected Autoware trajectory point,
and let $P_t^{aw}$ be the footprint obtained by transforming the `VehicleInfo`
local footprint by $\mathrm{pose}(A_t)$:
...
Let $\mathcal{L}^{route}$ be route-relevant lanelets collected from the trajectory
poses through `RouteHandler`. These are mission-route lanelets, not NAVSIM's full
drivable-area semantic layers.

The implementation first builds a convex hull over all trajectory footprints:

$$
H^{aw}
=
\mathrm{convexHull}\left(\bigcup_t P_t^{aw}\right).
$$

Candidate lanelets are:

$$
\mathcal{C}^{aw}
=
\{\ell\in\mathcal{L}^{route}\;|\;\mathrm{polygon}(\ell)\cap H^{aw}\ne\varnothing\}.
$$

The drivable proxy polygon set is the lanelet union:

$$
D^{aw}
=
\bigcup_{\ell\in\mathcal{C}^{aw}}\mathrm{polygon}(\ell).
$$
...
\mathbf{1}
\left[
\forall t,\;\mathrm{within}(P_t^{aw},D^{aw})
\right].
$$

If the trajectory, lanelets, vehicle info, or fused polygons are unavailable, the local
metric is marked unavailable rather than returning a NAVSIM-equivalent score.

**Main input gap.** NAVSIM asks whether ego corners remain inside broad drivable-area
semantic polygons. The migrated Autoware code asks whether every selected-trajectory
footprint is within a fused union of route-relevant lanelet polygons. This can be
stricter than NAVSIM because route lanelets are not the same set as all drivable area.

### Assessment

The migrated DAC is **not equation-identical** to NAVSIM. It is a route-lanelet-based proxy for NAVSIM's cached drivable-area-map test.
...
- **Migrated local helpers:** `calculate_raw_progress_m()`, `find_selected_candidate_index()`, `calculate_trajectory_similarity()`
...
| Required Input (NAVSIM) | Semantic Meaning (NAVSIM) | Autoware Replacement | Semantic Meaning (AW) | Judgement / Impact |
| :--- | :--- | :--- | :--- | :--- |
| **Simulator Proposal Batch** | Standardized set of alternative paths for the car. | `autoware_internal_planning_msgs::msg::CandidateTrajectories` | The alternative paths generated by the active planner. | **High.** Score depends on planner quality; a "bad" candidate set can inflate the score. |
...
- **Semantic replacement meaning:** the migration uses planner-emitted candidate trajectories as the proposal set, not the NAVSIM simulation proposal batch.
...
- NAVSIM normalizes raw progress using the best **multiplicatively valid** proposal:
  ...
  $$
- The migrated code normalizes against the best raw progress among all valid candidate trajectories, without multiplicative masking.
- The migrated code first matches the selected trajectory to the nearest candidate trajectory by a hand-built similarity score.
- **Impact:** **High.** Even with similar centerline geometry, the normalization denominator and proposal set are different.
...
**Migrated Autoware inputs.** The migrated implementation receives one selected
Autoware trajectory $q$ plus planner candidate trajectories $\{c_j\}$. For any
trajectory $q$, route-lanelet progress is:
...
The selected trajectory is matched to the most similar candidate trajectory. For
candidate $c_j$:
...
where $N_j=\min(|q|,|c_j|)$. The selected candidate index is:
...
=
\begin{cases}
\mathrm{clip}\left(\dfrac{p_{sel}^{aw}}{p_{best}^{aw}},0,1\right), &
p_{best}^{aw}>5.0 \\
1.0, & p_{best}^{aw}\le 5.0.
\end{cases}
$$

The migrated denominator does not multiply candidate progress by NC/DAC/DDC/TLC.

**Main input gap.** NAVSIM normalizes by the best progress among proposals that also
survive multiplicative subscores. Migrated Autoware normalizes by best raw route
progress among candidate trajectories and then assigns the selected trajectory to the
closest candidate by a pointwise similarity heuristic.
...
**NAVSIM inputs.** For proposal $i$, time $t$, and future offset
$\delta\in\{0,0.3,0.6,0.9\}$ seconds, NAVSIM forward-projects ego under current
velocity to a future polygon $\widehat{P}_{i,t,\delta}$ and compares it with
observation objects at $t+\delta$. The objects are the non-reactive scenario future
tracked objects, interpolated to the scorer grid.
...
=
\left[
\widehat{P}_{i,t,\delta}\cap O_{t+\delta,o}\ne\varnothing
\right].
$$
...
Let $\mathrm{Ahead}_{i,t,\delta,o}$ and $\mathrm{Behind}_{i,t,\delta,o}$ be the nuPlan
relative-angle predicates at the checked states. TTC fails if:
...
projection of ego. Let $\widehat{P}_{t,\delta}^{aw}$ be ego footprint at trajectory
time $t$ projected by current ego world velocity for future offset
$\delta\in\{0,0.3,0.6,0.9\}$.

Object pose is interpolated from the highest-confidence predicted path at query time
$t+\delta$, yielding object polygon $O_{t+\delta,o}^{aw}$.
