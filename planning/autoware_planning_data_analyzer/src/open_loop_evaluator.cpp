  ...
  rosbag2_cpp::Writer * bag_writer)
{
  ...
  if (route_handler_ && route_handler_->isHandlerReady()) {
    ...
    if (!evaluation_data_list.empty()) {
      (void)route_handler_->getRoadLaneletsAtPose(evaluation_data_list.front().synchronized_data->trajectory->points.front().pose);
    }
  }
      ...
      try {
        ...
        if (i == 0U) {
          ...
          const auto human_extended_comfort = metrics::calculate_extended_comfort(
            evaluation_data_list[i - 1U].ground_truth_trajectory,
  ...
  for (std::size_t i = 0; i < total_samples; ++i) {
    if (bag_writer) {
      save_metrics_to_bag(
        metrics_list_[i], synthetic_epdms_metrics_list_[i], evaluation_data_list[i],
        *bag_writer);
